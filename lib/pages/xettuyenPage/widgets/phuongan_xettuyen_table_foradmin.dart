...
import 'list_task_tt_table.dart';

class PhuonganXettuyenTableForAdmin extends StatefulWidget {
  final String dotdangkiId;
  const PhuonganXettuyenTableForAdmin({super.key, required this.dotdangkiId});

  @override
  State<PhuonganXettuyenTableForAdmin> createState() =>
      _PhuonganXettuyenTableForAdminState();
}

class _PhuonganXettuyenTableForAdminState
    extends State<PhuonganXettuyenTableForAdmin> {
  int _totalRow = 0;
        ...
        DataCell(ConstrainedColumnTable(
            child: Text(
                GlobalVariable.fromJsonStatusTrungtuyenPage[item.status] ??
                    ''))),
        DataCell(ConstrainedColumnTable(
            child: ListTaskTrungTuyenTable(thiSinhModel: item))),
      ],
    );
  }
